...
当用户选择在线对战时，SuperCube 会连接联网对战服务器 `wss://match.supercubegame.com`。为了完成匹配和实时同步，服务器可能处理连接 IP、连接时间、房间或对局 ID、临时玩家名称、协议版本、连接状态、对战同步消息、所选自定义方块/阵容数据、错误日志和基础安全日志。这些信息用于实时对战、同步、反作弊、安全和排障，不用于跨 App 或跨网站追踪，也不会出售给第三方。
...
SuperCube 的在线对战目前不要求账号登录，也不包含聊天、语音、好友系统或公开个人主页。对战服务器主要保存运行中的连接、排队和对局状态；这些状态用于完成当前对战，并可能在连接断开、对局结束、服务重启或排障完成后被清理。
...
如果未来接入新的统计、登录、支付、广告、服务器监控或客服服务，我们会在本政策中说明。
...
当前版本没有账号系统，因此暂不提供独立的账号删除入口。

## 8. 政策更新

当 SuperCube 增加账号、云存档、排行榜、好友、聊天、统计、付费或其他新功能时，本政策可能更新。更新后的政策会在公开页面或 App 说明中展示新的生效日期。
...
SuperCube online multiplayer currently does not require account login and does not include chat, voice, friend systems, or public user profiles. The match server primarily stores active connection, queue, and match state. These states are used to complete the current match and may be cleared after disconnection, match completion, service restart, or troubleshooting.
...
The current version does not include an account system, so there is no separate account deletion flow.

## 8. Changes

We may update this policy when SuperCube adds accounts, cloud saves, leaderboards, friends, chat, analytics, payments, or other new features. The updated policy will show a new effective date.
